...
import GoogleStitchClient from "./GoogleStitchClient";

export const revalidate = 60;

export const metadata: Metadata = {
  title: "Google Stitch Review 2026 — RankedAI™",
  description:
    "See how Google Stitch ranks in the Design & Build category on RankedAI™. Google's AI-powered web design tool — free during beta.",
  openGraph: {
    title: "Google Stitch Review 2026 — RankedAI™",
    description:
      "Google Stitch is an AI-powered web design tool from Google. See independent rankings, scores, and community reviews on RankedAI™.",
    type: "website",
  },
};

export default async function GoogleStitchPage() {
  const supabase = await createClient();

  const [{ data: authData }, { data: toolRow }] = await Promise.all([
    supabase.auth.getUser(),
    supabase.from("tools").select("id").eq("slug", "google-stitch").maybeSingle(),
  ]);
